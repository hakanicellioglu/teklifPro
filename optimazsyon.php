...
if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'])) {
    ...
    if ($glassInfo && $glassInfo['quantity'] > 0) {
        ...
        echo '<div>Genişlik: ' . e(number_format($glassInfo['width'], 0, ',', '.')) . ' mm, Yükseklik: ' . e(number_format($glassInfo['height'], 0, ',', '.')) . ' mm, Adet: ' . e(number_format($glassInfo['quantity'], 0, ',', '.')) . ', Toplam m²: ' . e(number_format($totalArea, 2, ',', '.')) . '</div>';
        echo '</div>';
    }
    if (!empty($customer)) {
        $customerName = trim(($customer['company_name'] ?? '') !== '' ? $customer['company_name'] : (($customer['first_name'] ?? '') . ' ' . ($customer['last_name'] ?? '')));
        echo '<div class="text-end"><strong>Müşteri:</strong> ' . e($customerName) . '</div>';
    }
    echo '</div>';

    echo '<div class="row row-cols-auto g-2 mb-3">';
    echo '<div class="col"><div class="card p-2"><small>Genişlik</small><div>' . e(number_format($row['width'], 0, ',', '.')) . ' mm</div></div></div>';
    echo '<div class="col"><div class="card p-2"><small>Yükseklik</small><div>' . e(number_format($row['height'], 0, ',', '.')) . ' mm</div></div></div>';
    echo '<div class="col"><div class="card p-2"><small>Adet</small><div>' . e(number_format($row['quantity'], 0, ',', '.')) . '</div></div></div>';
    if (!empty($row['motor_system'])) { echo '<div class="col"><div class="card p-2"><small>Motor</small><div>' . e($row['motor_system']) . '</div></div></div>'; }
    if (!empty($row['remote_quantity'])) { echo '<div class="col"><div class="card p-2"><small>Kumanda</small><div>' . e(number_format((int) $row['remote_quantity'], 0, ',', '.')) . '</div></div></div>'; }
    if (!empty($row['ral_code'])) { echo '<div class="col"><div class="card p-2"><small>RAL</small><div>' . e($row['ral_code']) . '</div></div></div>'; }
    echo '<div class="col"><div class="card p-2"><small>Sistem (kg)</small><div>' . e(number_format($systemWeightKg, 2, ',', '.')) . '</div></div></div>';
    echo '<div class="col"><div class="card p-2"><small>Alüminyum (kg)</small><div>' . e(number_format($result['alu_kg'], 2, ',', '.')) . '</div></div></div>';
    echo '<div class="col"><div class="card p-2"><small>Cam (m²)</small><div>' . e(number_format($totalArea, 2, ',', '.')) . '</div></div></div>';
    echo '</div>';
    ...
    //
    // Kategori Döngüsü
    // Her kategori için ürünleri kart şeklinde listeler.
    //
    foreach ($categories as $cat) {
        echo '<h5>' . e($cat['title']) . '</h5>';
        echo '<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-3 mb-3">';
        foreach ($cat['lines'] as $line) {
            $prod = $provider->getProduct($line['name']);
            $code = $prod['product_code'] ?? '';
            $img  = $prod['image_url'] ?? '';
            $qtyDisplay = isset($line['pieces']) && $line['pieces'] > 0 ? $line['pieces'] : $line['quantity'];
            echo '<div class="col">';
            echo '<div class="card h-100">';
            echo '<div class="card-body p-2">';
            echo '<h6 class="card-title">' . e($line['name']) . '</h6>';
            echo '<div class="small">Kod: ' . e($code) . '</div>';
            echo '<div class="small">Ölçü: ' . e(number_format($line['measure'], 0, ',', '.')) . ' mm</div>';
            echo '<div class="small">Adet: ' . e(number_format($qtyDisplay, 0, ',', '.')) . '</div>';
            echo '</div>';
            if ($img) {
                echo '<img src="' . e($img) . '" class="card-img-bottom img-fluid" alt="' . e($line['name']) . '">';
            }
            echo '</div>';
            echo '</div>';
        }
