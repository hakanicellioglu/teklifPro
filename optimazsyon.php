...
if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'])) {
    ...
    class PdoProductProvider implements ProductProviderInterface
    {
        ...
        public function getProduct(string $name): ?array
        {
            $stmt = $this->pdo->prepare('SELECT p.product_code AS code, p.unit, p.unit_price, p.weight_per_meter, p.price_unit, p.image_url, c.name AS category FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE LOWER(p.name) = LOWER(:name)');
